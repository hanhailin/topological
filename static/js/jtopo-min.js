        ...
        function b(c) {
            this.initialize = function (c) {
                ...
                this.paint = function (a) {
                    ...
                    this.paintEchoText(a)
                },
                /*   增加报警换行  http://darkjs.com
                ...
                /*
                 //增加echo标签输出
                 this.paintEchoText = function(a) {
                 this.paintEchoText = function(a) {
                 if (null != this.echo && "" != this.echo) {
                 var b = this.echoColor || "0,255,255",
                 c = this.echoAlpha || .5;
                 a.beginPath(),
                 a.font = this.echoFont || "10px 微软雅黑";
                 var textArray = this.echo.split('\n');
                 textArray.unshift('\n')
                 textArray.push('\n')
                 var rowCnt = textArray.length;
                 var i = 0, imax = rowCnt, maxLength = 0;
                 ...
                 var lineHeight = a.measureText("元").width;
                 //算出echo的最大的宽度
                 var d = ((a.measureText(this.echo).width / rowCnt + 6) > maxWidth ? (a.measureText(this.echo).width / rowCnt + 6) : maxWidth);
                 var e = a.measureText("田").width,
                 f = this.width / 2 - d / 2,
                 g = (-this.height / 2 - e * rowCnt ) - 8;
                 e = (e) * rowCnt;
                ...
                this.paintEchoText = function (a) {
                    if (null != this.echo && "" != this.echo) {
                        // var b = this.echoColor || "0,255,255",
                                                var b = this.echoColor || "199,238,206",

                            c = this.echoAlpha || 1;
                            echoFontColor = '123,104,238',
                        a.beginPath(),
                            a.font = this.echoFont || "10px 微软雅黑";
                        var d = a.measureText(this.echo).width,    //文本字符串的宽度
                            e = a.measureText("田").width,         //单位字符的宽度+6

                            g = -this.height / 2 - e;             //相对高度
                            h = 150,                                     //文本换行最大长度
                            f = this.width / 2 - h / 2,               //node的x坐标减去字符的宽度一半，得到起始坐标
                            n = f + 2,                                     //x右移2位
                            l = 0,                                        //初始坐标初始化
                            p = 0,
                            m = 0,                                      //初始化游标
                            r = 0;                                      //剩余初始值
                            s = 0;
                        for (var i = 0; i < this.echo.length; i++) {
                            if (this.echo[i] == ' ') {
                                k = i
                                p = p + 1
                            }
                            m += a.measureText(this.echo[i]).width;
                            if (m > h) {
                                if (p == 0) {
                                    o += (e + 8);
                                    m = 0;
                                    ...
                                    o += (e + 8);
                                    for(var q=k+1;q<i+1;q++){
                                        r += a.measureText(this.echo[q]).width;
                                    }
                                    m = r
                                    r = 0
                                    l = k+1
                            ...
                            a.lineWidth = 1,
                            a.moveTo(f - 10, o - 20),
                            a.lineTo(f + 20 + h, o - 20),
                            a.lineTo(f + 20 + h, g + e - 10),
                            a.lineTo(f + h / 2 + 6, g + e - 10),
                            a.lineTo(f + h / 2, g + e + 8 - 10),
                            a.lineTo(f + h / 2 - 6, g + e - 10),
                            a.lineTo(f - 10, g + e - 10),
                            a.lineTo(f - 10, g - 20),
                            a.fill(),
                            a.stroke(),
                            a.closePath(),
                            a.beginPath(),
                            a.strokeStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")","#0000ff"
                            // a.fillStyle = "rgba(" + this.fontColor + ", " + this.alpha + ")"
                            a.fillStyle = "rgba(" + echoFontColor + ", " + this.alpha + ")"
                        ...
                        for (var i = 0; i < this.echo.length; i++) {
                            ...
                            if (m > h) {
                                if (p == 0) {
                                    a.fillText(this.echo.substring(l, i), n, o),
                                    o += (e + 8);
                                    m = 0;
                                    l = i;
                                } else {
                                    a.fillText(this.echo.substring(l, k), n, o),
                                    o += (e + 8);
                                    for(var q=k+1;q<i+1;q++){
                                        r += a.measureText(this.echo[q]).width;
                                    }
                                ...
                                if (i == this.echo.length - 1) {
                                    a.fillText(this.echo.substring(l, i + 1), n, o);
                                }
                            }
                            a.closePath()
                        }
